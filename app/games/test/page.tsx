              ...
              <div className="flex flex-col gap-1.5">
                {(['all', 'mastered', 'backlog'] as const).map((tab) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-bold capitalize transition-all ${selectedTab === tab
              ...
              <div className="flex flex-col gap-1.5">
                {(Object.keys(MAIN_CATEGORY_REGISTRY) as Array<keyof typeof MAIN_CATEGORY_REGISTRY>).map((key) => {
                  ...
                  return (
                    <label
                      key={key}
                      className={`relative w-full overflow-hidden p-[1px] rounded-lg cursor-pointer select-none transition-all duration-300 active:scale-[0.99] block group ${isSelected ? 'shadow-md shadow-black/30' : ''
                        }`}
                    >
                      <div className={`absolute inset-0 rounded-lg transition-opacity duration-300 ${isSelected
                          ? `bg-gradient-to-br ${catConfig.bgGradient} opacity-100`
                          : 'bg-slate-900/60 group-hover:bg-slate-800 opacity-100'
                        }`} />

                      <div className={`relative px-2.5 py-1.5 rounded-[7px] flex items-center gap-2 text-[11px] font-black uppercase tracking-wide transition-all duration-300 ${isSelected
