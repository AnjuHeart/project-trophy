...
// Map categories to match your database mainCategoryId names
const MAIN_CATEGORY_REGISTRY: Record<string, { label: string; emoji: string; bgGradient: string }> = {
    ...
    // Add other exact DB category IDs here
};
...
export default function FilterSidebar({
    ...
    return (
        <>
            ...
            <aside className={`
                fixed inset-y-0 left-0 z-50 w-[85vw] max-w-sm h-screen overflow-y-auto transform transition-transform duration-300 ease-in-out p-5 shadow-2xl
                ${isOpen ? 'translate-x-0' : '-translate-x-full'}
                bg-gradient-to-br from-slate-950 to-slate-900 border-r border-slate-900
                lg:translate-x-0 lg:static lg:h-auto lg:w-auto lg:block lg:sticky lg:top-24 lg:bg-gradient-to-br lg:from-slate-900 lg:to-slate-950/40
                lg:border lg:border-slate-900/80 lg:rounded-2xl lg:shadow-xl`}
            >
                ...
                <div className="flex items-center justify-between lg:hidden mb-6 pb-4 border-b border-slate-900">
                    ...
                    <button
                        onClick={onClose}
                        className="w-7 h-7 flex items-center justify-center text-slate-400 hover:text-white bg-slate-950 border border-slate-900 rounded-lg text-xs font-bold"
                    >
                        ✕
                    </button>
                </div>

                {/* SECTION 1: DYNAMIC CATEGORY FILTER */}
                ...
                {/* SECTION 2: AWARDS GROUP (HALL OF FAME CATEGORIES) */}
                <div className="space-y-3">
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-black text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">
                            {activeMaxTime ? `${activeMaxTime}h` : 'Any Time'}
                        </span>
                    </div>
                    ...
                    <input
                        ...
                        max="150"
                        step="5"
                        value={activeMaxTime || 150}
                        onChange={(e) => {
                            const val = Number(e.target.value);
                            onMaxTimeChange(val >= 150 ? null : val); // If slider hits max max, remove query constraint entirely
                        }}
