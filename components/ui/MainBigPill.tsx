    ...
    return (
        <div className="flex flex-col gap-1.5 w-full">
            <div className={`p-[1px] rounded-xl bg-gradient-to-br ${mainAndGameData.bgGradient} ${mainAndGameData.hoverGradient} shadow-2xl transition-all duration-300 hover:scale-[1.005] group/card w-full`}>
                <div className={`transition-colors duration-300 rounded-[11px] p-4 md:p-5 space-y-2 md:space-y-4 ${isFlashing ? 'bg-slate-950/70' : 'bg-slate-950/85 group-hover/card:bg-slate-950/70'}`}>
                    ...
                    <div className="space-y-1.5 md:space-y-2">
                        ...
                        >
                            <div className={`transition-[max-height] duration-400 ease-in-out overflow-hidden 
                                ${isExpanded ? "max-h-[500px]" : "max-h-[2.4rem] md:max-h-none"}`}>
                                <p 
                                    ref={textRef}
                                    className={`text-[11px] md:text-xs font-medium text-slate-300 leading-relaxed pr-4 md:pr-0 ${isExpanded ? "" : "line-clamp-2 md:line-clamp-none"}`}
                                >
                                    {mainAndGameData.description}
